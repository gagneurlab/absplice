...
def outlier_dl5():
    return SpliceOutlierDataloader(
        fasta_file, vcf_file, ref_tables5=[ref_table5_kn_testis, ref_table5_kn_lung],
...
def outlier_dl3():
    return SpliceOutlierDataloader(
        fasta_file, vcf_file, ref_tables3=[ref_table3_kn_testis])
...
def test_splicing_outlier_dataloader_init(outlier_dl):
    ...
    assert list(set(outlier_dl.combined_ref_tables5.ref_tables[0]['tissue'])) == ['testis']
    ...
    assert list(set(outlier_dl.combined_ref_tables3.ref_tables[0]['tissue'])) == ['testis']
