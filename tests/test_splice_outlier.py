...
def test_outlier_results_infer_cat(outlier_results, cat_dl, outlier_model):
    ...
    assert sorted(results.gene.columns.tolist()) == sorted([
        ...
        'End', 'Strand', 'events', 'splice_site', 'ref_psi', 'k', 'n', 'median_n', 
        'novel_junction', 'weak_site_acceptor', 'weak_site_donor',
    ...
    assert results.junction.loc[(
        '17:41201211-41203079:-', 'NA00002', 'lymphocytes')] is not None



def test_outlier_results_infer_cat_fake(outlier_dl_fake, cat_dl_fake, outlier_model):
# def test_outlier_results_infer_cat(outlier_model):

    results = outlier_model.predict_on_dataloader(outlier_dl_fake)
    results.infer_cat(cat_dl_fake)
