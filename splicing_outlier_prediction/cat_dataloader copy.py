...
class CatInference(RefTableMixin):
    ...
    def __init__(self, splicing_outlier_result, count_cat, **kwargs):
        ...
        self.splicing_outlier_result5 = splicing_outlier_result.df[
            splicing_outlier_result.df['event_type'] == 'psi5']
        self.splicing_outlier_result3 = splicing_outlier_result.df[
            splicing_outlier_result.df['event_type'] == 'psi3']

        if self.splicing_outlier_result5 is not None:
            self.common_junctions5 = set(self.splicing_outlier_result5.junction) \
        ...
        if self.splicing_outlier_result3 is not None:
            self.common_junctions3 = set(self.splicing_outlier_result3.junction) \
                .intersection(self.ct.junctions)
    ...
    def infer(self, junction_id, sample, event_type, clip_threshold=0.01):
        ...
        if event_type == 'psi5':
            ...
            ref_tables = self.splicing_outlier_result5.set_index('junction')
        elif event_type == 'psi3':
            ct_cat = self.ct_cat3
            psi_cat = ct_cat.psi3
            ref_psi_cat = self.ref_psi3_cat
            ref_tables = self.splicing_outlier_result3.set_index('junction')
        else:
